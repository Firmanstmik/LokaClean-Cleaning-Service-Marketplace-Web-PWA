...
import { Package, Clock, MapPin, CreditCard, CheckCircle2, AlertCircle, ArrowRight, Calendar, ChevronRight, ChevronLeft, Filter, List, Sparkles } from "lucide-react";
...
export function OrdersPage() {
  ...
  return (
    <div className="w-full bg-gradient-to-br from-tropical-50 via-ocean-50/50 to-sand-50/70">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-7 space-y-4 sm:space-y-6 pb-6 sm:pb-8 lg:pb-12">
      ...
      {loading ? (
        ...
        {items.map((o, index) => {
          ...
          return (
            <AnimatedCard key={o.id} delay={index * 0.1}>
              <Link
                to={`/orders/${o.id}`}
                className={`card-lombok relative block group max-w-full ${
                  isActive 
              ...
              <motion.div
                ...
                style={{
                  background: 'linear-gradient(135deg, rgba(37,99,235,0.05) 0%, rgba(6,182,212,0.08) 50%, rgba(37,99,235,0.05) 100%)',
              ...
              <motion.div
                className="absolute inset-0 rounded-2xl sm:rounded-3xl bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-0 group-hover:opacity-100 pointer-events-none"
                animate={{
                  x: ['-100%', '200%'],
                }}
                transition={{
              ...
              {/* Inner shine overlay */}
              <div className="absolute inset-0 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-white/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
              ...
              <div className="relative z-10 p-4 sm:p-5 lg:p-6">
                <div className="flex flex-wrap items-start justify-between gap-3 sm:gap-4">
                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-center gap-2">
                      <motion.div
                        ...
                        className={`flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-lg sm:rounded-xl bg-gradient-to-br ${getPackageGradient(o.paket.name)} shadow-lg shadow-black/10 flex-shrink-0 relative overflow-hidden group/icon`}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-[10px] sm:text-xs font-semibold text-slate-500 group-hover:text-slate-600 transition-colors duration-300">{t("orders.orderNumber")} #{o.order_number}</div>
                  ...
                  <div className="flex flex-col items-end gap-1.5 sm:gap-2 flex-shrink-0">
                    ...
                    <div className={`inline-flex items-center gap-1 sm:gap-1.5 rounded-lg px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs font-semibold ${getPaymentStatusColor(o.pembayaran.status)}`}>
                      ...
                      <span className="text-slate-600 hidden sm:inline whitespace-nowrap">{o.pembayaran.method}</span>
                    </div>
                  </div>
                </div>

                <motion.div
                  className="mt-3 sm:mt-4 flex items-center justify-end"
                  initial={{ opacity: 0.7 }}
                  whileHover={{ opacity: 1 }}
                  transition={{ duration: 0.2 }}
                >
                  <motion.button
                    className="group/view relative flex items-center gap-2 px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl sm:rounded-2xl bg-gradient-to-r from-blue-500 via-cyan-500 to-blue-500 text-white text-xs sm:text-sm font-bold shadow-[0_4px_16px_rgba(37,99,235,0.25),0_2px_8px_rgba(6,182,212,0.2)] hover:shadow-[0_6px_24px_rgba(37,99,235,0.35),0_4px_12px_rgba(6,182,212,0.25)] transition-all duration-300 overflow-hidden"
                    style={{
                      backgroundSize: '200% 200%',
                    }}
                  ...
                  >
                    ...
                    {/* Content */}
                    <span className="relative z-10 font-bold tracking-wide drop-shadow-[0_1px_2px_rgba(0,0,0,0.2)]">
